...
const URL = 'http://localhost:4173/esc_7/'
...
const browser = await puppeteer.launch({
  ...
  args: ['--no-sandbox', '--window-size=1280,720', '--autoplay-policy=no-user-gesture-required'],
...
try {
  ...
  const testResume = async () => {
    const before = await page.evaluate(() => JSON.parse(localStorage.getItem('esc7-save-v1') ?? 'null'))
    ...
    await sleep(900)
    const st = await page.evaluate(() => JSON.parse(localStorage.getItem('esc7-save-v1') ?? 'null'))
  ...
  const saveGone = await page.evaluate(() => localStorage.getItem('esc7-save-v1') === null)
