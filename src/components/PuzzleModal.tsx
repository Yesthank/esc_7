...
/** 홍길동전 테마 미끼 음절 풀 — 인물·장소·기물에서 추출(정답 음절과 중복 시 런타임 제외).
 *  금칙 안전: '부'·'형' 은 풀에서 제외(노출 조합 위험 원천 차단, §3). */
const THEME_SYLLABLES: readonly string[] = [
  '홍', '길', '동', '활', '빈', '당', '별', '초', '승', '달',
  '도', '적', '굴', '해', '인', '사', '감', '영', '대', '궐',
  '복', '주', '머', '니', '제', '웅', '호', '패', '윷', '말',
  '산', '가', '책', '력', '문', '갑', '단', '청', '오', '방',
...
function buildSlate(puzzleId: string, answer: string): string[] {
  ...
  const pool = THEME_SYLLABLES.filter((s) => !answerSyls.includes(s))
  const decoys: string[] = []
