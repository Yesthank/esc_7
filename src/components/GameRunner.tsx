...
const INTERSTITIAL_SEEN = 'inter-apsong'
...
export function GameRunner({ config, clueMap }: { config: GameConfig; clueMap: ClueMap }) {
  ...
  // R1 별당 밤 앰비언트(바람·풀벌레·까마귀) — hg-pouch 해정 = 어머니와 하직, 까마귀가 영구히 멎는다.
  const pouchSolved = state.flags['hg-pouch-solved'] === true
  useCrowAmbience(
  ...
  const onHotspot = (h: Hotspot) => {
    ...
    if (a.type === 'examine') {
      ...
      if (a.roomId === DAEGWOL && !state.seenDialogs.includes(INTERSTITIAL_SEEN)) {
        setInterTo(a.roomId)
        return
      }
